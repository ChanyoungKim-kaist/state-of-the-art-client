    ...
    <v-row>
        ...
        <v-flex text-start>
            <v-hover v-slot="{hover}">
                <v-btn class="startbtn blue yellow--text" rounded v-if="hover" @click="gameStart">Game Start</v-btn>
                <v-btn class="startbtn yellow red--text" rounded v-else @click="gameStart">Game Start</v-btn>
            </v-hover>
            <v-hover v-slot="{hover}">
                <v-text v-if="hover"
                    class="gametext white--text">score : <span id='score'>0</span>
                </v-text>
                <v-text v-else
                    class="gametext">score : <span id='score'>0</span>
                </v-text>
            </v-hover>
        <!-- </v-flex>
    ...
    if(openCardId1 == ''){
        ...
        if(openCardSrc1 == openCardSrc2) {  //일치
            ...
            if(++openedCtn == 8){
                ...
                axios.post("http://192.249.18.172:80/game", {score: score * 100}, this.config)
                    .then( res => {
                        if (!res.data.ok) {
                            alert ('점수 등록에 실패했습니다.')
                        } 
